...
def test_value_wit_text():
    with pytest.raises(ValueError,
                       match=r'The number P must be greater than 1'):
        is_prime(1)
...
def test_warning_false(capfd):
    ...
    assert captured.out == ('If the number P is prime, the check may take a '
                            'long time. Working...\n'
                            )
...
if __name__ == '__main__':
    ...
    pytest.main(['test_pytest_02.py'])  # -v  - аналог флага verbous=True
    # pytest.main(['-v', 'test_pytest_02.py'])
